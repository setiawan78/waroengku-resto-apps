import { createLikeWaroengkuButtonTemplate, createunLikedWaroengkuButtonTemplate } from '../views/templates/template-creator';

const LikeButtonPresenter = {
  async init({ likeButtonContainer, favoriteRestos, resto }) {
    this._likeButtonContainer = likeButtonContainer;
    this._resto = resto;
    this._favoriteRestos = favoriteRestos;

    await this._renderButton();
  },

  async _renderButton() {
    const { id } = this._resto;

    if (await this._isWaroengkuExist(id)) {
      this._renderLiked();
    } else {
      this._renderLike();
    }
  },

  async _isWaroengkuExist(id) {
    const resto = await this._favoriteRestos.getWaroengku(id);
    return !!resto;
  },

  _renderLike() {
    this._likeButtonContainer.innerHTML = createLikeWaroengkuButtonTemplate();

    const likeButton = document.querySelector('#likeButton');
    likeButton.addEventListener('click', async () => {
      await this._favoriteRestos.putWaroengku(this._resto);
      this._renderButton();
    });
  },

  _renderLiked() {
    this._likeButtonContainer.innerHTML = createunLikedWaroengkuButtonTemplate();

    const likeButton = document.querySelector('#likeButton');
    likeButton.addEventListener('click', async () => {
      await this._favoriteRestos.deleteWaroengku(this._resto.id);
      this._renderButton();
    });
  },
};

export default LikeButtonPresenter;
